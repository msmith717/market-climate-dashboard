...
treas_w = to_weekly_window(fetch_curve_wide(TREASURY_SERIES), years=3)
ig_w = to_weekly_window(fetch_curve_wide(IG_SERIES), years=3)

# Only allow weeks where BOTH exist
common = pd.merge(treas_w[["date"]], ig_w[["date"]], on="date", how="inner").sort_values("date")
dates_ts = common["date"].tolist()
N = len(dates_ts)

# Fixed y-axis (bottom at 0) across whole dataset
...
if st.session_state.nom_playing:
    st_autorefresh(interval=900, key="nom_autoplay")
...
# Keep 6M / 12M for the spread chart below
treas_6m = nearest_row(treas_w, now_date - pd.DateOffset(months=6))
treas_12m = nearest_row(treas_w, now_date - pd.DateOffset(months=12))
ig_6m = nearest_row(ig_w, now_date - pd.DateOffset(months=6))
ig_12m = nearest_row(ig_w, now_date - pd.DateOffset(months=12))
...
fig3 = go.Figure()
show_now_anchor = not st.session_state.nom_playing

# Selected week (moving): both gray
